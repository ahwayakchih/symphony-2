...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en">
	...
	$warnings = array(
		...
		'database-incorrect-version' => __('Symphony requires <code>MySQL 4.1</code> (or <code>SQLite 2.8.0</code>) or greater to work. This requirement must be met before installation can proceed.'),
	...
	Class Action{
		...
		function requirements(&$Page){
			...
			if(checkRequirement('mysql_connect', 'func', true)) $drivers['mysql'] = mysql_get_client_info();
			if(checkRequirement('sqlite_open', 'func', true)) $drivers['sqlite'] = sqlite_libversion();

			if(!isset($drivers['mysql']) && !isset($drivers['sqlite'])){
				$Page->log->pushToLog('Requirement - Database extension (MySQL or SQLite) not present' , E_ERROR, true);
				$missing[] = MISSING_SQL;
			}

			elseif(!isset($drivers['mysql']) && !checkRequirement($drivers['sqlite'], 'version', '2.8.0')){
				$Page->log->pushToLog('Requirement - SQLite Version is not correct. '.$drivers['sqlite'].' detected.' , E_ERROR, true);
				$missing[] = MISSING_SQL;
			}

			elseif(!isset($drivers['sqlite']) && !checkRequirement($drivers['mysql'], 'version', '4.1')){
				$Page->log->pushToLog('Requirement - MySQL Version is not correct. '.$drivers['mysql'].' detected.' , E_ERROR, true);
				$missing[] = MISSING_SQL;
			}
		...
		function install(&$Page, $fields){
			...
			try{
				...
				elseif($fields['database']['driver'] == 'mysql'){
					...
					$Page->log->pushToLog("Configuration - Bad Database Engine Specified: " . $fields['database']['driver'], E_NOTICE, true);
				...
				if($db->isConnected())
					$tables = $db->fetch(sprintf(
							...
							$db->cleanValue($fields['database']['name']),
							$db->cleanValue($fields['database']['prefix']) . '%'
						));
			}
			...
			elseif($fields['database']['driver'] == 'mysql' && version_compare($db->fetchVar('version', 0, "SELECT VERSION() AS `version`;"), '4.1', '<')){
			...
			## Otherwise there are no error, proceed with installation
				...
				$install_log->pushToLog("Database: Establishing Connection...", E_NOTICE, true, false);
				...
				$install_log->pushToLog("Database: Selecting Database '".$database['name']."'...", E_NOTICE, true, false);
				...
				$install_log->pushToLog("Database: Importing Table Schema...", E_NOTICE, true, false);
				$error = NULL;
				...
				$install_log->pushToLog("Database: Creating Default Author...", E_NOTICE, true, false);
				if(!$db->query($author_sql)){
					$error = $db->getLastError();
					define('_INSTALL_ERRORS_', "There was an error while trying create the default author. Database returned: " . $error['num'] . ': ' . $error['msg']);
					$install_log->pushToLog("Failed", E_ERROR, true, true, true);
					installResult($Page, $install_log, $start);
...
';
				...
				if(@!is_dir($fields['docroot'] . '/workspace')){
					...
					$install_log->pushToLog("Database: Importing Workspace Data...", E_NOTICE, true, false);
					$error = NULL;
